import sys
sys.path.append('..')
sys.path.append('D:/Samuel_VitDepth/DEReD/DEReD')
import numpy as np
    ...
    def eval_model(self, far=None):
        ...
        for i, batch in enumerate(tqdm(dataloader)):
            ...
            if not self.args.gt_dpt:
                ...
                for j in range(pred_dpt.size(0)):
                    depth_map_path = os.path.join(self.eval_savepath, f'pred_dpt_{i}_{j}.png')
                    plt.imsave(depth_map_path, pred_dpt[j].cpu().numpy(), cmap='viridis')
            if not self.args.gt_aif:
                pred_aif = torch.clip(raw_aif, 0, 1)
                MG, SF = eval_aif(pred_aif.cpu())
                df_model['MG'].append(MG.numpy())
                df_model['SF'].append(SF.numpy())
                # Save all-in-focus images
                for j in range(pred_aif.size(0)):
                    aif_path = os.path.join(self.eval_savepath, f'aif_{i}_{j}.png')
                    plt.imsave(aif_path, pred_aif[j].permute(1, 2, 0).cpu().numpy())
